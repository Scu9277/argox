...
[ "$1" = "del" ] || [ "$1" = "list" ] || [ "$1" = "proxy" ] || [ "$vwp" = yes ] || [ "$sop" = yes ] || [ "$vxp" = yes ] || [ "$ssp" = yes ] || [ "$vlp" = yes ] || [ "$vmp" = yes ] || [ "$hyp" = yes ] || [ "$tup" = yes ] || [ "$xhp" = yes ] || [ "$anp" = yes ] || [ "$arp" = yes ] || { echo "提示：未安装argosbx脚本，请在脚本前至少设置一个协议变量哦，再见！💣"; exit; }
fi
export uuid=${uuid:-''}
...
echo "                                                        "; 
    ...
    echo "=================================================="
    echo " "
    echo -e "     作者: ${GREEN}Scu x Duang 周周独家定制版${NC}"
...
if [ -n "$groups_name" ]; then
echo "$groups_name" > "$HOME/agsbx/groups_name"
echo "代理组名称：$groups_name"
fi
if [ "$pxen" = "yes" ]; then
    if [ -z "$pxtp" ] || [ -z "$pxip" ] || [ -z "$pxpt" ]; then
        echo "提示：启用代理(pxen=yes)时，必须同时设置代理类型(pxtp)、IP(pxip)和端口(pxpt)！"
        exit 1
    fi
    if [ "$pxtp" != "socks" ] && [ "$pxtp" != "http" ]; then
         echo "提示：代理类型(pxtp)必须为 'socks' 或 'http'！"
         exit 1
    fi
    mkdir -p "$HOME/agsbx"
    cat > "$HOME/agsbx/proxy.conf" <<EOF
PROXY_TYPE="$pxtp"
PROXY_IP="$pxip"
PROXY_PORT="$pxpt"
PROXY_USER="$pxus"
...
PROXY_ENABLED="yes"
EOF
    echo "已通过环境变量配置并启用出口代理。"
fi
load_proxy_config(){
    if [ -f "$HOME/agsbx/proxy.conf" ]; then
        . "$HOME/agsbx/proxy.conf"
    else
        PROXY_ENABLED="no"
    fi
}
...
proxy_menu(){
    echo "=================================================="
    echo "            出口代理配置 (Outbound Proxy)"
    echo "=================================================="
    echo "当前状态: $([ "$PROXY_ENABLED" = "yes" ] && echo "${GREEN}已启用${NC}" || echo "${RED}已禁用${NC}")"
    ...
    echo "--------------------------------------------------"
    echo "1. 配置/修改 代理 (Configure Proxy)"
    echo "2. 启用/禁用 代理 (Toggle Proxy)"
    echo "0. 退出 (Exit)"
    echo "=================================================="
    ...
    case "$choice" in
        1)
            ...
            read -p "请输入用户名 (留空则无认证): " PROXY_USER
        ...
        2)
            if [ "$PROXY_ENABLED" = "yes" ]; then
                ...
                if [ -f "$HOME/agsbx/proxy.conf" ]; then
                     ...
                     if grep -q "PROXY_IP" "$HOME/agsbx/proxy.conf"; then
                        PROXY_ENABLED="yes"
                        echo "代理已启用。"
                     else
                        echo "未找到有效配置，请先配置代理。"
            ...
            # Update config file status only
            if [ -f "$HOME/agsbx/proxy.conf" ]; then
                sed -i '/PROXY_ENABLED/d' "$HOME/agsbx/proxy.conf"
                echo "PROXY_ENABLED=\"$PROXY_ENABLED\"" >> "$HOME/agsbx/proxy.conf"
            fi
            echo "正在重新应用配置..."
            ins
            ;;
...
v4v6
if [ "$PROXY_ENABLED" = "yes" ]; then
    s1outtag=proxy-out; s2outtag=proxy-out; x1outtag=proxy-out; x2outtag=proxy-out; xip='"::/0", "0.0.0.0/0"'; sip='"::/0", "0.0.0.0/0"'; wap=proxy
    echo; echo "已启用出口代理"
elif echo "$v6" | grep -q '^2a09' || echo "$v4" | grep -q '^104.28'; then
...
if ! "$HOME/agsbx/sing-box" version >/dev/null 2>&1; then
    echo "Sing-box binary download failed or is corrupted. Retrying..."
    rm "$HOME/agsbx/sing-box"
    (command -v curl>/dev/null 2>&1 && curl -Lo "$out" -# --retry 2 "$url") || (command -v wget>/dev/null 2>&1 && timeout 3 wget -O "$out" --tries=2 "$url")
    chmod +x "$HOME/agsbx/sing-box"
    if ! "$HOME/agsbx/sing-box" version >/dev/null 2>&1; then
        echo "Sing-box binary is still corrupted. Please check your network connection."
        echo "Binary header: $(head -n 1 "$HOME/agsbx/sing-box" | cut -c 1-50)"
    fi
fi
...
insuuid(){
    # If uuid is still empty here, try to read from file
    if [ -z "$uuid" ] && [ -e "$HOME/agsbx/uuid" ]; then
        uuid=$(cat "$HOME/agsbx/uuid")
    fi
    
    # If still empty, force regeneration
    if [ -z "$uuid" ]; then
        # Try system methods first (more reliable)
        uuid=$(cat /proc/sys/kernel/random/uuid 2>/dev/null || uuidgen 2>/dev/null)
        
        # If system methods fail, try binaries
        if [ -z "$uuid" ] && [ -e "$HOME/agsbx/xray" ] && "$HOME/agsbx/xray" version >/dev/null 2>&1; then
            uuid=$("$HOME/agsbx/xray" uuid)
        elif [ -z "$uuid" ] && [ -e "$HOME/agsbx/sing-box" ] && "$HOME/agsbx/sing-box" version >/dev/null 2>&1; then
    ...
    if [ -z "$uuid" ]; then
         echo "Error: Failed to generate UUID. Using a default one for debugging."
...
if [ ! -e "$HOME/agsbx/xray" ] || ! "$HOME/agsbx/xray" version >/dev/null 2>&1; then
upxray
fi
...
echo
echo "=========启用Sing-box内核========="
if [ ! -e "$HOME/agsbx/sing-box" ] || ! "$HOME/agsbx/sing-box" version >/dev/null 2>&1; then
upsingbox
fi
        ...
        "servers": [
          {
            "address": "${PROXY_IP}",
            "port": ${PROXY_PORT}
            $( [ -n "$PROXY_USER" ] && echo ", \"users\": [{ \"user\": \"$PROXY_USER\", \"pass\": \"$PROXY_PASS\" }]" )
          }
        ]
      }
    },
...
if [ "$PROXY_ENABLED" = "yes" ]; then
        ...
        cat >> "$HOME/agsbx/sb.json" <<EOF
    {
      ...
      "version": "5"
      $( [ -n "$PROXY_USER" ] && echo ", \"username\": \"$PROXY_USER\", \"password\": \"$PROXY_PASS\"" )
    },
EOF
    else
        cat >> "$HOME/agsbx/sb.json" <<EOF
...
mkdir -p "$HOME/bin"
cp "$0" "$SCRIPT_PATH" 2>/dev/null || (command -v curl >/dev/null 2>&1 && curl -sL "$agsbxurl" -o "$SCRIPT_PATH") || (command -v wget >/dev/null 2>&1 && wget -qO "$SCRIPT_PATH" "$agsbxurl")
chmod +x "$SCRIPT_PATH"
...
echo "Argosbx脚本进程未启动，安装失败"
echo "=== Xray Log ==="
cat "$HOME/agsbx/xray.log" 2>/dev/null
journalctl -u xr -n 20 --no-pager 2>/dev/null
echo "=== Sing-box Log ==="
cat "$HOME/agsbx/sb.log" 2>/dev/null
journalctl -u sb -n 20 --no-pager 2>/dev/null
echo "=== Xray Config ==="
cat "$HOME/agsbx/xr.json" 2>/dev/null
echo "=== Sing-box Config ==="
cat "$HOME/agsbx/sb.json" 2>/dev/null
exit
fi
}
argosbxstatus(){
...
ipchange
rm -rf "$HOME/agsbx/jh.txt" "$HOME/agsbx/clash.yaml"
cat > "$HOME/agsbx/clash.yaml" <<EOF
proxies:
EOF
rm -f "$HOME/agsbx/proxy_names.txt"
uuid=$(cat "$HOME/agsbx/uuid")
server_ip=$(cat "$HOME/agsbx/server_ip.log")
sxname=$(cat "$HOME/agsbx/name" 2>/dev/null)
groups_name=$(cat "$HOME/agsbx/groups_name" 2>/dev/null)
...
port_xh=$(cat "$HOME/agsbx/port_xh")
vl_xh_link="vless://$uuid@$server_ip:$port_xh?encryption=$enkey&flow=xtls-rprx-vision&security=reality&sni=$ym_vl_re&fp=chrome&pbk=$public_key_x&sid=$short_id_x&type=xhttp&path=$uuid-xh&mode=auto#${name:-${sxname}vl-xhttp-reality-$hostname}"
echo "$vl_xh_link" >> "$HOME/agsbx/jh.txt"
echo "$vl_xh_link"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vl-xhttp-reality-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vl-xhttp-reality-$hostname}
    type: vless
    server: $server_ip
    port: $port_xh
    uuid: $uuid
...
port_vx=$(cat "$HOME/agsbx/port_vx")
vl_vx_link="vless://$uuid@$server_ip:$port_vx?encryption=$enkey&flow=xtls-rprx-vision&type=xhttp&path=$uuid-vx&mode=auto#${name:-${sxname}vl-xhttp-$hostname}"
...
vl_vx_cdn_link="vless://$uuid@yg$(cfip).ygkkk.dpdns.org:$port_vx?encryption=$enkey&flow=xtls-rprx-vision&type=xhttp&host=$xvvmcdnym&path=$uuid-vx&mode=auto#${name:-${sxname}vl-xhttp-cdn-$hostname}"
echo "$vl_vx_cdn_link" >> "$HOME/agsbx/jh.txt"
echo "$vl_vx_cdn_link"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vl-xhttp-cdn-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vl-xhttp-cdn-$hostname}
    type: vless
    server: yg$(cfip).ygkkk.dpdns.org
    port: $port_vx
    uuid: $uuid
    network: xhttp
    tls: false
...
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vl-xhttp-$hostname}
    ...
    xhttp-opts:
      ...
      path: /$uuid-vx
EOF
[ -n "$name" ] && echo "      - \"$name\"" >> "$HOME/agsbx/proxy_names.txt" || echo "      - \"vl-xhttp-$hostname\"" >> "$HOME/agsbx/proxy_names.txt"
fi
if grep vless-ws "$HOME/agsbx/xr.json" >/dev/null 2>&1; then
echo "💣【 Vless-ws-enc 】支持ENC加密，节点信息如下："
port_vw=$(cat "$HOME/agsbx/port_vw")
vl_vw_link="vless://$uuid@$server_ip:$port_vw?encryption=$enkey&flow=xtls-rprx-vision&type=ws&path=$uuid-vw#${name:-${sxname}vl-ws-enc-$hostname}"
...
echo "注：默认地址 yg数字.ygkkk.dpdns.org 可自行更换优选IP域名，如是回源端口需手动修改443或者80系端口"
vl_vw_cdn_link="vless://$uuid@yg$(cfip).ygkkk.dpdns.org:$port_vw?encryption=$enkey&flow=xtls-rprx-vision&type=ws&host=$xvvmcdnym&path=$uuid-vw#${name:-${sxname}vl-ws-enc-cdn-$hostname}"
echo "$vl_vw_cdn_link" >> "$HOME/agsbx/jh.txt"
echo "$vl_vw_cdn_link"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vl-ws-enc-cdn-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vl-ws-enc-cdn-$hostname}
    type: vless
    server: yg$(cfip).ygkkk.dpdns.org
    port: $port_vw
    uuid: $uuid
...
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vl-ws-enc-$hostname}
    ...
    ws-opts:
      path: /$uuid-vw
EOF
[ -n "$name" ] && echo "      - \"$name\"" >> "$HOME/agsbx/proxy_names.txt" || echo "      - \"vl-ws-enc-$hostname\"" >> "$HOME/agsbx/proxy_names.txt"
fi
if grep reality-vision "$HOME/agsbx/xr.json" >/dev/null 2>&1; then
echo "💣【 Vless-tcp-reality-vision 】节点信息如下："
port_vl_re=$(cat "$HOME/agsbx/port_vl_re")
vl_link="vless://$uuid@$server_ip:$port_vl_re?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$ym_vl_re&fp=chrome&pbk=$public_key_x&sid=$short_id_x&type=tcp&headerType=none#${name:-${sxname}vl-reality-vision-$hostname}"
echo "$vl_link" >> "$HOME/agsbx/jh.txt"
echo "$vl_link"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vl-reality-vision-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vl-reality-vision-$hostname}
    type: vless
    server: $server_ip
    port: $port_vl_re
    uuid: $uuid
...
port_ss=$(cat "$HOME/agsbx/port_ss")
ss_link="ss://$(echo -n "2022-blake3-aes-128-gcm:$sskey@$server_ip:$port_ss" | base64 -w0)#${name:-${sxname}Shadowsocks-2022-$hostname}"
echo "$ss_link" >> "$HOME/agsbx/jh.txt"
echo "$ss_link"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - Shadowsocks-2022-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-Shadowsocks-2022-$hostname}
    type: ss
    server: $server_ip
    port: $port_ss
...
port_vm_ws=$(cat "$HOME/agsbx/port_vm_ws")
vm_link="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vm-ws-$hostname}\", \"add\": \"$server_ip\", \"port\": \"$port_vm_ws\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"www.bing.com\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
...
echo "注：默认地址 yg数字.ygkkk.dpdns.org 可自行更换优选IP域名，如是回源端口需手动修改443或者80系端口"
vm_cdn_link="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vm-ws-cdn-$hostname}\", \"add\": \"yg$(cfip).ygkkk.dpdns.org\", \"port\": \"$port_vm_ws\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$xvvmcdnym\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vm_cdn_link" >> "$HOME/agsbx/jh.txt"
echo "$vm_cdn_link"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vm-ws-cdn-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vm-ws-cdn-$hostname}
    type: vmess
    server: yg$(cfip).ygkkk.dpdns.org
    port: $port_vm_ws
    uuid: $uuid
  ...
  - name: ${name:-vm-ws-$hostname}
    ...
    ws-opts:
      ...
      headers:
        Host: www.bing.com
EOF
[ -n "$name" ] && echo "      - \"$name\"" >> "$HOME/agsbx/proxy_names.txt" || echo "      - \"vm-ws-$hostname\"" >> "$HOME/agsbx/proxy_names.txt"
fi
if grep anytls-sb "$HOME/agsbx/sb.json" >/dev/null 2>&1; then
echo "💣【 AnyTLS 】节点信息如下："
port_an=$(cat "$HOME/agsbx/port_an")
an_link="anytls://$uuid@$server_ip:$port_an?insecure=1&allowInsecure=1#${name:-${sxname}anytls-$hostname}"
...
port_ar=$(cat "$HOME/agsbx/port_ar")
ar_link="anytls://$uuid@$server_ip:$port_ar?security=reality&sni=$ym_vl_re&fp=chrome&pbk=$public_key_s&sid=$short_id_s&type=tcp&headerType=none#${name:-${sxname}any-reality-$hostname}"
echo "$ar_link" >> "$HOME/agsbx/jh.txt"
echo "$ar_link"
echo
# Any-Reality is complex to map to standard Clash, skipping or mapping as VLESS Reality if compatible
# Assuming it's similar to VLESS Reality for client
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-any-reality-$hostname}
    type: vless
    server: $server_ip
    port: $port_ar
    uuid: $uuid
...
port_hy2=$(cat "$HOME/agsbx/port_hy2")
hy2_link="hysteria2://$uuid@$server_ip:$port_hy2?security=tls&alpn=h3&insecure=1&sni=www.bing.com#${name:-${sxname}hy2-$hostname}"
echo "$hy2_link" >> "$HOME/agsbx/jh.txt"
echo "$hy2_link"
echo
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-hy2-$hostname}
    type: hysteria2
    server: $server_ip
    port: $port_hy2
    password: $uuid
    ...
    alpn:
      - h3
EOF
[ -n "$name" ] && echo "      - \"$name\"" >> "$HOME/agsbx/proxy_names.txt" || echo "      - \"hy2-$hostname\"" >> "$HOME/agsbx/proxy_names.txt"
fi
if grep tuic5-sb "$HOME/agsbx/sb.json" >/dev/null 2>&1; then
echo "💣【 Tuic 】节点信息如下："
port_tu=$(cat "$HOME/agsbx/port_tu")
tuic5_link="tuic://$uuid:$uuid@$server_ip:$port_tu?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=www.bing.com&allow_insecure=1&allowInsecure=1#${name:-${sxname}tuic-$hostname}"
echo "$tuic5_link" >> "$HOME/agsbx/jh.txt"
echo "$tuic5_link"
echo
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-tuic-$hostname}
    type: tuic
    server: $server_ip
    port: $port_tu
    uuid: $uuid
...
EOF
[ -n "$name" ] && echo "      - \"$name\"" >> "$HOME/agsbx/proxy_names.txt" || echo "      - \"tuic-$hostname\"" >> "$HOME/agsbx/proxy_names.txt"
fi
...
echo "客户端密码：$uuid"
echo
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - Socks5-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-Socks5-$hostname}
    type: socks5
    server: $server_ip
    port: $port_so
    username: $uuid
...
argodomain=$(cat "$HOME/agsbx/sbargoym.log" 2>/dev/null)
[ -z "$argodomain" ] && argodomain=$(grep -a trycloudflare.com "$HOME/agsbx/argo.log" 2>/dev/null | awk 'NR==2{print}' | awk '{print $2}' | sed 's/^[ \t]*//;s/[ \t]*$//')
if [ -n "$argodomain" ]; then
vlvm=$(cat $HOME/agsbx/vlvm 2>/dev/null)
if [ "$vlvm" = "Vmess" ]; then
vmatls_link1="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-tls-argo-$hostname-443}\", \"add\": \"yg1.ygkkk.dpdns.org\", \"port\": \"443\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link1" >> "$HOME/agsbx/jh.txt"
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vmess-ws-tls-argo-$hostname-443" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vmess-ws-tls-argo-$hostname-443}
    type: vmess
    ...
    ws-opts:
      ...
      headers:
        Host: $argodomain
EOF
vmatls_link2="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-tls-argo-$hostname-8443}\", \"add\": \"yg2.ygkkk.dpdns.org\", \"port\": \"8443\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link2" >> "$HOME/agsbx/jh.txt"
vmatls_link3="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-tls-argo-$hostname-2053}\", \"add\": \"yg3.ygkkk.dpdns.org\", \"port\": \"2053\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link3" >> "$HOME/agsbx/jh.txt"
vmatls_link4="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-tls-argo-$hostname-2083}\", \"add\": \"yg4.ygkkk.dpdns.org\", \"port\": \"2083\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link4" >> "$HOME/agsbx/jh.txt"
vmatls_link5="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-tls-argo-$hostname-2087}\", \"add\": \"yg5.ygkkk.dpdns.org\", \"port\": \"2087\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link5" >> "$HOME/agsbx/jh.txt"
vmatls_link6="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-tls-argo-$hostname-2096}\", \"add\": \"[2606:4700::0]\", \"port\": \"2096\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"tls\", \"sni\": \"$argodomain\", \"alpn\": \"\", \"fp\": \"chrome\"}" | base64 -w0)"
echo "$vmatls_link6" >> "$HOME/agsbx/jh.txt"
vma_link7="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-80}\", \"add\": \"yg6.ygkkk.dpdns.org\", \"port\": \"80\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link7" >> "$HOME/agsbx/jh.txt"
vma_link8="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-8080}\", \"add\": \"yg7.ygkkk.dpdns.org\", \"port\": \"8080\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link8" >> "$HOME/agsbx/jh.txt"
vma_link9="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-8880}\", \"add\": \"yg8.ygkkk.dpdns.org\", \"port\": \"8880\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link9" >> "$HOME/agsbx/jh.txt"
vma_link10="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-2052}\", \"add\": \"yg9.ygkkk.dpdns.org\", \"port\": \"2052\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link10" >> "$HOME/agsbx/jh.txt"
vma_link11="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-2082}\", \"add\": \"yg10.ygkkk.dpdns.org\", \"port\": \"2082\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link11" >> "$HOME/agsbx/jh.txt"
vma_link12="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-2086}\", \"add\": \"yg11.ygkkk.dpdns.org\", \"port\": \"2086\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link12" >> "$HOME/agsbx/jh.txt"
vma_link13="vmess://$(echo "{ \"v\": \"2\", \"ps\": \"${name:-${sxname}vmess-ws-argo-$hostname-2095}\", \"add\": \"[2400:cb00:2049::0]\", \"port\": \"2095\", \"id\": \"$uuid\", \"aid\": \"0\", \"scy\": \"auto\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"$argodomain\", \"path\": \"/$uuid-vm\", \"tls\": \"\"}" | base64 -w0)"
echo "$vma_link13" >> "$HOME/agsbx/jh.txt"
elif [ "$vlvm" = "Vless" ]; then
vwatls_link1="vless://$uuid@yg$(cfip).ygkkk.dpdns.org:443?encryption=$enkey&flow=xtls-rprx-vision&type=ws&host=$argodomain&path=$uuid-vw&security=tls&sni=$argodomain&fp=chrome&insecure=0&allowInsecure=0#${name:-${sxname}vless-ws-tls-argo-enc-vision-$hostname}"
echo "$vwatls_link1" >> "$HOME/agsbx/jh.txt"
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vless-ws-tls-argo-enc-vision-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vless-ws-tls-argo-enc-vision-$hostname}
    type: vless
    server: yg$(cfip).ygkkk.dpdns.org
    port: 443
    uuid: $uuid
    ...
    ws-opts:
      ...
      headers:
        Host: $argodomain
EOF
vwa_link2="vless://$uuid@yg$(cfip).ygkkk.dpdns.org:80?encryption=$enkey&flow=xtls-rprx-vision&type=ws&host=$argodomain&path=$uuid-vw&security=none#${name:-${sxname}vless-ws-argo-enc-vision-$hostname}"
echo "$vwa_link2" >> "$HOME/agsbx/jh.txt"
[ -n "$name" ] && echo "      - $name" >> "$HOME/agsbx/proxy_names.txt" || echo "      - vless-ws-argo-enc-vision-$hostname" >> "$HOME/agsbx/proxy_names.txt"
cat >> "$HOME/agsbx/clash.yaml" <<EOF
  - name: ${name:-vless-ws-argo-enc-vision-$hostname}
    type: vless
    server: yg$(cfip).ygkkk.dpdns.org
    port: 80
    uuid: $uuid
...
echo "========================================================="
echo "YAML 配置 (Clash Meta):"
# Generate proxy-groups
echo "proxy-groups:" >> "$HOME/agsbx/clash.yaml"
echo "  - name: ${groups_name:-Proxy-Group}" >> "$HOME/agsbx/clash.yaml"
echo "    type: select" >> "$HOME/agsbx/clash.yaml"
echo "    proxies:" >> "$HOME/agsbx/clash.yaml"
# Use collected names
cat "$HOME/agsbx/proxy_names.txt" >> "$HOME/agsbx/clash.yaml"
rm -f "$HOME/agsbx/proxy_names.txt"
